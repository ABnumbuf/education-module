...
def get_keys_ElGamal(A: int, B: int) -> Tuple[int, int, int, int]:
    # Генерирует открытый и приватный кдючи для ЦП Эль-Гамаля
    #
...
def ds_ElGamal_outp(m: str, p: int, g: int, x: int) -> List[str]:
    # Создает ЦП Эль-Гамаля сообщения m
    #
    outp = []
    h = my_hash(m, p)
    outp.append(f'h = h(m) = {h}\n')
    k = 5  # get_coprime_in_range(2, p - 1, p - 1)
    outp.append(f'k = {k}\n')
    r = binary_pow(g, k, p)
